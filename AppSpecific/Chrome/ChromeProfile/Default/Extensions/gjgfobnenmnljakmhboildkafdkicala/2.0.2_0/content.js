...
communicator.on("InjectOwnJavascriptLibrary", function (message, responseFunction) {
    try {
...
communicator.on("InjectOwnRuntimeJavascriptLibrary", function (message, responseFunction) {
    try {
...
communicator.on("TravelThruHistory", function (message, responseFunction) {
    try {
        if (message.arg0)
            window.history.back();
        else
...
communicator.on("GetDimensions", function (message, responseFunction) {
    try {
...
communicator.on("RunScript", function (message, responseFunction) {
    try {
